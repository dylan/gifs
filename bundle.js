...
var accessKeyID = "AKIAJSCHVMKXUBSNYH2A";
var secretAccessKey = "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";
...
var filteredItems = ["index.html", "robots.txt", "bundle.js"];
...
function sortDate(mode, a, b) {
    var _ref = [a.lastModified.getTime(), b.lastModified.getTime()],
        aTime = _ref[0],
        bTime = _ref[1];
...
function sortName(mode, a, b) {
    var _ref2 = [a.name.toLowerCase(), b.name.toLowerCase()],
        aName = _ref2[0],
        bName = _ref2[1];
    ...
    return h(
        ...
        { key: item.name,
            onmouseenter: function onmouseenter(_) {
                return actions.hover(item);
            },
            onmouseleave: function onmouseleave(_) {
                return actions.hover(null);
            }
        },
        h(
            'td',
            ...
            h(
                'a',
                { href: 'http://' + params.Bucket + '/' + item.name },
                item.name
            )
        ),
        h(
            'td',
            null,
            hooks(item.lastModified).format("YYYY MMM Do")
        )
    );
};
...
app({
    ...
    view: function view(state, actions) {
        var items = state.items.sort(function (a, b) {
            ...
            return !filteredItems.includes(item.name.toLowerCase());
        }).filter(function (item) {
        ...
        return h(
            'main',
            null,
            h(Viewer, { item: state.hoveredItem }),
            h('input', { placeholder: 'Search...', type: 'text', oninput: actions.search }),
            h(
                'table',
                { 'class': 'sortTable' },
                h(
                    ...
                    h(
                        ...
                        h(
                            ...
                            { onclick: function onclick(_) {
                                    return actions.setSort(sortName);
                                }, 'class': state.sortFunction == sortName ? 'active' : null },
                            'Name'
                        )
                    ),
                    h(
                        ...
                        h(
                            'a',
                            { onclick: function onclick(_) {
                                    return actions.setSort(sortDate);
                                }, 'class': state.sortFunction == sortDate ? 'active' : null },
                            'Date'
                        )
                    )
                )
            ...
            h(
                ...
                items.map(function (item) {
                    return h(ListItem, { item: item, actions: actions, hovered: item == state.hoveredItem });
                })
            )
        );
    },
    ...
    actions: {
        ...
        hover: function hover(state, actions, item) {
            return { hoveredItem: item };
        },

        setSort: function setSort(state, actions, sortFunction) {
        ...
        search: function search(state, actions, event) {
            return { filter: event.target.value.trim().toLowerCase() };
        },

        loadBucket: function loadBucket(state, actions) {
        ...
        updateItems: function updateItems(state, actions, data) {
            return { items: data.Contents.map(function (obj) {
                    return { name: obj.Key, lastModified: obj.LastModified };
                }) };
        }
    },
